...
describe('Vet CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/vets')
					...
					.end(function(vetSaveErr, vetSaveRes) {
						...
						// Update existing Vet
						agent.put('/vets/' + vetSaveRes.body._id)
							.send(vet)
							.expect(200)
	...
	it('should be able to get a single Vet if not signed in', function(done) {
		...
		// Save the Vet
		vetObj.save(function() {
			request(app).get('/vets/' + vetObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/vets')
					...
					.end(function(vetSaveErr, vetSaveRes) {
						...
						// Delete existing Vet
						agent.delete('/vets/' + vetSaveRes.body._id)
							.send(vet)
							.expect(200)
	...
	it('should not be able to delete Vet instance if not signed in', function(done) {
		...
		vetObj.save(function() {
			// Try deleting Vet
			request(app).delete('/vets/' + vetObj._id)
			.expect(401)
			.end(function(vetDeleteErr, vetDeleteRes) {
				// Set message assertion
				(vetDeleteRes.body.message).should.match('User is not logged in');
